        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
            ...
            <div className="glass p-8 rounded-[2.5rem] border-white/5 relative overflow-hidden group hover:border-luxury-gold/20 transition-all">
                ...
                <div className="flex items-center gap-3 mb-6 relative z-10">
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white">Agenda de Hoje</h3>
                </div>

                <div className="space-y-4 relative z-10">
                    {data.meetings.length === 0 ? (
                        <p className="text-sm italic opacity-60 text-white">Sem reuniões agendadas.</p>
                    ) : (
                        data.meetings.map(m => (
                            <div key={m.id} className="flex gap-4 items-start p-4 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors">
                                <div className="text-center min-w-[3rem]">
                                    ...
                                    <span className="block text-[11px] uppercase tracking-wider opacity-50 text-white">H</span>
                                </div>
                                <div>
                                    <h4 className="text-sm font-serif italic text-white leading-tight mb-1">{m.title}</h4>
                                    <div className="flex items-center gap-2 opacity-60 text-[11px] uppercase tracking-wider text-white">
            ...
            {/* Coluna 2: Tarefas Prioritárias */}
            <div className="glass p-8 rounded-[2.5rem] border-white/5 relative overflow-hidden group hover:border-luxury-gold/20 transition-all">
                <div className="flex items-center gap-3 mb-6">
                    <div className="p-2 bg-luxury-gold/10 rounded-xl text-luxury-gold">
                        <CheckSquare size={18} />
                    </div>
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white">Foco Imediato</h3>
                </div>
                        ...
                        <div key={task.id} className="group/task flex items-center justify-between p-4 bg-luxury-gold/5 border border-luxury-gold/10 rounded-2xl hover:bg-luxury-gold/10 transition-all cursor-pointer">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] uppercase tracking-wider opacity-60 text-white mt-0.5 flex items-center gap-2">
                                        {task.projectKey} • <span className="text-red-400 flex items-center gap-1"><Clock size={8} /> {task.deadline}</span>
                                    </p>
                                </div>
                            </div>
            ...
            {/* Coluna 3: Alertas e Métricas de Risco */}
            <div className="glass p-8 rounded-[2.5rem] border-white/5 relative overflow-hidden group hover:border-red-500/20 transition-all bg-red-500/[0.02]">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white">Atenção Necessária</h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 h-[calc(100%-3rem)]">
                    <div className="bg-white/5 rounded-2xl p-4 flex flex-col justify-between border border-white/5 hover:border-red-500/30 transition-colors cursor-pointer">
                        ...
                        <span className="text-[11px] uppercase tracking-widest opacity-60 text-white leading-tight">Faturas<br />Vencidas</span>
                    </div>
                    <div className="bg-white/5 rounded-2xl p-4 flex flex-col justify-between border border-white/5 hover:border-red-500/30 transition-colors cursor-pointer">
                        <span className="text-3xl font-serif text-white">{data.stalledProjects}</span>
                        <span className="text-[11px] uppercase tracking-widest opacity-60 text-white leading-tight">Projetos<br />Parados</span>
                    </div>
                    ...
                    <button className="col-span-2 mt-auto py-3 px-4 bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-between group/btn border border-white/5">
                        <span className="text-[11px] uppercase tracking-widest opacity-60 text-white">Ver Relatório de Riscos</span>
